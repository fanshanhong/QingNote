...
class _FilterPanelState extends ConsumerState<FilterPanel> {
  ...
  Future<List<_FilterRow>> _buildRows() async {
    ...
    rows.add(_PseudoRow(
        ...
        await noteRepo.count(filter: NoteListFilter.uncategorized),
        NoteListFilter.uncategorized));
    rows.add(_PseudoRow(
        '我的收藏',
        Icons.star_border,
        await noteRepo.count(filter: NoteListFilter.favorite),
        NoteListFilter.favorite));
    rows.add(_PseudoRow(
        '最近删除',
        Icons.delete_outline,
        await noteRepo.count(filter: NoteListFilter.deleted),
        NoteListFilter.deleted));
    rows.add(const _DividerRow());
    rows.add(const _SectionHeaderRow());

    for (final folder in await folderRepo.list()) {
      final fCount =
          await noteRepo.count(filter: NoteListFilter.folder(folder.id));
      ...
      if (expanded) {
        for (final nb in await notebookRepo.listByFolder(folder.id)) {
          final nbCount =
              await noteRepo.count(filter: NoteListFilter.notebook(nb.id));
  ...
  Widget _buildRowWidget(
      BuildContext ctx, _FilterRow row, NoteListFilter current) {
    return switch (row) {
  ...
  bool _filtersEqual(NoteListFilter a, NoteListFilter b) {
    ...
    if (a is NotebookFilter && b is NotebookFilter)
      return a.notebookId == b.notebookId;
    return false;
  }

  Widget _buildPseudoTile(
      BuildContext ctx, _PseudoRow r, NoteListFilter current) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(children: [
          Icon(r.icon,
              size: 20, color: sel ? AppColors.primary : AppColors.textPrimary),
  ...
  Widget _buildNotebookTile(
      BuildContext ctx, _NotebookRow r, NoteListFilter current) {
