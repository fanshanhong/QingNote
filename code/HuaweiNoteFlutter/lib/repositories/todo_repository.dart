...
class TodoRepository {
  ...
  Future<int> count({int? folderId, bool includeDeleted = false}) async {
    ...
    final result = await db.rawQuery(
      'SELECT COUNT(*) FROM todos WHERE ${where.join(' AND ')}',
      args,
    );
  ...
  Future<void> softDelete(int id) async {
    final db = await _dbHelper.database;
    await db.update(
        'todos', {'deleted_at': DateTime.now().millisecondsSinceEpoch},
  ...
  Future<void> softDeleteBatch(List<int> ids) async {
    ...
    await db.transaction((txn) async {
      for (final id in ids) {
        await txn.update('todos', {'deleted_at': now},
            where: 'id = ?', whereArgs: [id]);
      }
    });
  }
  ...
  Future<void> restore(int id) async {
    final db = await _dbHelper.database;
    await db.update('todos', {'deleted_at': 0},
        where: 'id = ?', whereArgs: [id]);
  }
  ...
  static DateTime _addMonth(DateTime dt, int months) {
    ...
    return DateTime(
        targetYear, targetMonth, day, dt.hour, dt.minute, dt.second);
  }
